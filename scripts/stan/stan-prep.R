...
fl_rates <- read_csv("data/state-pop-totals.csv") %>%
  ...
  mutate( # next, add to model: (black, white) ~ noraml(); brate=bsents/black etc.
            brate = bsents/black,
            wrate = wsents/white) %>%
  dplyr::select(year, white, black, bsents, wsents, total_sents) %>%
...
dcounty <- read_csv("data/constructed/sentencing.csv") %>%
  dplyr::select(county, year, sentences, population_black, population_white) %>%
  inner_join(fl_rates, by = "year") %>%
  dplyr::select(year, county, sentences, population_black, population_white, wrate, brate) %>%
  rename("y" = sentences) %>%
  arrange(county) %>%
